...
class SQLiteIntegration {
	...
	function __construct() {
		...
		if (function_exists('register_uninstall_hook')) {
			register_uninstall_hook(__FILE__, ['SQLiteIntegration', 'uninstall']);
		}
		if (function_exists('is_multisite') && is_multisite()) {
			add_action('network_admin_menu', [$this, 'add_network_pages']);
			add_action('network_admin_notices', ['SQLiteIntegrationUtils', 'show_admin_notice']);
		} else {
			add_action('admin_menu', [$this, 'add_pages']);
			add_action('admin_notices', ['SQLiteIntegrationUtils', 'show_admin_notice']);
		}
		// See the docstring for download_backup_db() in utilities/utility.php
		// We need this registration process.
		add_action('admin_init', ['SQLiteIntegrationUtils', 'download_backup_db']);
	...
	function add_pages() {
		...
		if (function_exists('add_options_page')) {
			$welcome_page     = add_options_page(__('SQLite Integration'), __('SQLite Integration'), 'manage_options', 'sqlite-integration', [$utils, 'welcome']);
			$util_page        = add_submenu_page(null, 'System Info', 'System Info', 'manage_options', 'sys-info', [$utils, 'show_utils']);
			$edit_db          = add_submenu_page(null, 'Setting File', 'Setting File', 'manage_options', 'setting-file', [$utils, 'edit_db_file']);
			$doc_page         = add_submenu_page(null, 'Documentation', 'Documentation', 'manage_options', 'doc', [$doc, 'show_doc']);
			$patch_page       = add_submenu_page(null, 'Patch Utility', 'Patch Utility', 'manage_options', 'patch', [$patch_utils, 'show_patch_page']);
			$maintenance_page = add_submenu_page(null, 'DB Maintenance', 'DB Maintenance', 'manage_options', 'maintenance', [$maintenance, 'show_maintenance_page']);
			add_action('admin_print_styles-'.$welcome_page, [$this, 'add_style_sheet']);
			add_action('admin_print_styles-'.$util_page, [$this, 'add_style_sheet']);
			add_action('admin_print_styles-'.$edit_db, [$this, 'add_style_sheet']);
			add_action('admin_print_styles-'.$doc_page, [$this, 'add_style_sheet']);
			add_action('admin_print_styles-'.$patch_page, [$this, 'add_style_sheet']);
			add_action('admin_print_scripts-'.$util_page, [$this, 'add_sqlite_script']);
			add_action('admin_print_scripts-'.$doc_page, [$this, 'add_sqlite_script']);
			add_action('admin_print_scripts-'.$patch_page, [$this, 'add_sqlite_script']);
			add_action('admin_print_scripts-'.$edit_db, [$this, 'add_sqlite_script']);
			add_action('admin_print_styles-'.$maintenance_page, [$this, 'add_style_sheet']);
		}
	}
	...
	function add_network_pages() {
		global $utils, $doc, $patch_utils, $maintenance;
		if (function_exists('add_options_page')) {
			$welcome_page     = add_submenu_page('settings.php', __('SQLite Integration'), __('SQLite Integration'), 'manage_network_options', 'sqlite-integration', [$utils, 'welcome']);
			$util_page        = add_submenu_page(null, 'System Info', 'System Info', 'manage_network_options', 'sys-info', [$utils, 'show_utils']);
			$edit_db          = add_submenu_page(null, 'Setting File', 'Setting File', 'manage_network_options', 'setting-file', [$utils, 'edit_db_file']);
			$doc_page         = add_submenu_page(null, 'Documentation', 'Documentation', 'manage_network_options', 'doc', [$doc, 'show_doc']);
			$patch_page       = add_submenu_page(null, 'Patch Utility', 'Patch Utility', 'manage_network_options', 'patch', [$patch_utils, 'show_patch_page']);
			$maintenance_page = add_submenu_page(null, 'DB Maintenance', 'DB Maintenance', 'manage_network_options', 'maintenance', [$maintenance, 'show_maintenance_page']);
			add_action('admin_print_styles-'.$welcome_page, [$this, 'add_style_sheet']);
			add_action('admin_print_styles-'.$util_page, [$this, 'add_style_sheet']);
			add_action('admin_print_styles-'.$edit_db, [$this, 'add_style_sheet']);
			add_action('admin_print_styles-'.$doc_page, [$this, 'add_style_sheet']);

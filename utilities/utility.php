...
class SQLiteIntegrationUtils {
			...
			if (preg_match('/<h2>([^<]+)<\/h2>/', $info_array[$i], $match)) {
				...
				foreach ($info_array2 as $info) {
					...
					if (preg_match($pattern3, $info, $match)) {
						$modules[$name][trim($match[1])] = [trim($match[2]), trim($match[3])];
	...
	private function get_backup_files() {
		...
		$names_to_exclude = ['.', '..', '.htaccess', 'debug.txt', '.ht.sqlite', 'index.php', $db_name];
		$backup_files = [];
		if (is_dir(FQDBDIR)) {
	...
	static function download_backup_db() {
		...
		if (isset($_POST['download_backup_file']) && stripos($_SERVER['HTTP_REFERER'], $script_url) !== false) {
			...
			$blog_name = str_replace([' ', '　', ';'], ['_', '_', '_'], get_bloginfo('name'));
...
JS;
		...
		if (isset($_GET['page']) && $_GET['page'] == 'setting-file') :?>
			...
			?>
			<p><?php printf('<input type="submit" name="sqliteintegration_update_db_file" value="%s" onclick="return confirm(\'%s\')" class="button-primary">', __('Update', $domain), __('この内容を保存してもよろしいですか?\n\nやめるなら[キャンセル]を、続けるなら[OK]をクリック。' , $domain));?></p>
			</form>
			</div>
		<?php endif;
	}
